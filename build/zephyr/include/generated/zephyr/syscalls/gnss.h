...
extern int z_impl_gnss_start(const struct device * dev, enum gnss_start_mode mode);

static inline int gnss_start(const struct device * dev, enum gnss_start_mode mode)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
		union { uintptr_t x; enum gnss_start_mode val; } parm1 = { .val = mode };
		return (int) arch_syscall_invoke2(parm0.x, parm1.x, K_SYSCALL_GNSS_START);
	}
#endif
	compiler_barrier();
	return z_impl_gnss_start(dev, mode);
}

#if defined(CONFIG_TRACING_SYSCALL)
#ifndef DISABLE_SYSCALL_TRACING

#define gnss_start(dev, mode) ({ 	int syscall__retval; 	sys_port_trace_syscall_enter(K_SYSCALL_GNSS_START, gnss_start, dev, mode); 	syscall__retval = gnss_start(dev, mode); 	sys_port_trace_syscall_exit(K_SYSCALL_GNSS_START, gnss_start, dev, mode, syscall__retval); 	syscall__retval; })
#endif
#endif


extern int z_impl_gnss_stop(const struct device * dev);

static inline int gnss_stop(const struct device * dev)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
		return (int) arch_syscall_invoke1(parm0.x, K_SYSCALL_GNSS_STOP);
	}
#endif
	compiler_barrier();
	return z_impl_gnss_stop(dev);
}

#if defined(CONFIG_TRACING_SYSCALL)
#ifndef DISABLE_SYSCALL_TRACING

#define gnss_stop(dev) ({ 	int syscall__retval; 	sys_port_trace_syscall_enter(K_SYSCALL_GNSS_STOP, gnss_stop, dev); 	syscall__retval = gnss_stop(dev); 	sys_port_trace_syscall_exit(K_SYSCALL_GNSS_STOP, gnss_stop, dev, syscall__retval); 	syscall__retval; })
#endif
#endif
